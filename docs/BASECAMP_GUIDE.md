...
CLI, so you always build the CLI first and point the module at it.
...
Install the Risc0 toolchain with `rzup` (or `cargo install cargo-risczero
--version 3.0.5`). The module forces `RISC0_DEV_MODE=0`, so approving and
executing require the guest toolchain; creating, proposing, and reading state
do not.
...
The module resolves a `quorum` executable in two ways: it first tries the
`quorum` on your `PATH`, then falls back to the path stored in settings
(`~/.config/Logos/Quorum.conf`). You can override either in the UI:

1. Stay on the **Local** tab.
2. In **Runtime**, set **CLI binary** to the absolute path from step 1
   (e.g. `/path/to/LEZ-Quorum/target/release/quorum`).
...
against the CLI's filesystem state machine. It needs no sequencer and is the
fastest way to confirm the module works.
...
`https://testnet.lez.logos.co`, and requires a fresh private session
directory.
...
2. Click **Check RPC** to confirm health, then **Verify deployment** on
   **1 Setup**.
3. Click **Prepare private state**. This runs
